...
class Enlace:
    ...
    def __raw_recv(self, dados):
        ...
        if dados == b'':
            return None
        currentdata = dados.split(b'\xc0')
        if dados.endswith(b'\xc0'):
            self.buffer = b''
        else:
            self.buffer = currentdata[len(currentdata)-1]
        for i in range(len(currentdata)-1):
            currentdata[i] = currentdata[i].replace(b'\xdb\xdc', b'\xc0')
            currentdata[i] = currentdata[i].replace(b'\xdb\xdd', b'\xdb')
            if currentdata[i] != b'':
                try:
                    self.callback(currentdata[i])
                except:
                    import traceback
                    traceback.print_exc
                finally:
